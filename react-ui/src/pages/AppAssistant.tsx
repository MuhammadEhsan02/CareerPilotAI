...
// Header component
interface HeaderProps { refresh_key: number }
const Header: React.FC<HeaderProps> = ({ refresh_key }) => {
  return (
    <header className="atomize-header">
      <div className="logo-nav">
        {/* <img src={reactLogo} alt="Logo" className="logo" /> */}
  <span className="brand">CareerPilotAI</span>
      </div>
...
// SubHeader section
interface SubHeaderProps { onRun: () => void; thread: any; fileName: string; lastResumeHtml: string | null }
const SubHeader: React.FC<SubHeaderProps> = ({ onRun, thread, fileName, lastResumeHtml }) => (
...
const MyForm: React.FC<MyFormProps> = ({ userquery, setUserQuery, placeholder }) => {
...
interface UserInputCardProps { userquery: string; setUserQuery: (val: string) => void }
const UserInputCard: React.FC<UserInputCardProps> = ({ userquery, setUserQuery }) => (
...
interface DocumentInputCardProps { handleFileChange: (e: ChangeEvent<HTMLInputElement>) => void }
const DocumentInputCard: React.FC<DocumentInputCardProps> = ({ handleFileChange }) => (
  <div className="card document-input">
    <div 
      className="image-placeholder" 
      style={{
...
interface JobDescriptionCardProps { jobdetails: string; setJobDetails: (val: string) => void }
const JobDescriptionCard: React.FC<JobDescriptionCardProps> = ({ jobdetails, setJobDetails }) => (
  <div className="card document-input">
    <div 
      className="image-placeholder" 
      style={{
...
const LandingPageAtomize: React.FC = () => {
  const [userquery, setUserQuery] = useState('');
  const [fileName, setFileName] = useState('');
  const [fileBase64, setFileBase64] = useState<string | null>(null);
  const [jobdetails, setJobDetails] = useState('');
  const [lastResumeHtml, setLastResumeHtml] = useState<string | null>('');
  ...
  const thread = useStream<{ messages: Message[] }>({
    ...
    apiKey: session ? `Bearer ${session.access_token}` : undefined,
    onFinish: (state) => {
  ...
  return (
    <div className="atomize-root">
      ...
      <div className="cards-row">
        ...
        {/* Force re-mount of file input by changing a key on a wrapper div */}
        <div key={fileInputKey} className="file-input-wrapper">
